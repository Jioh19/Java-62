...
public class ConfiguracionDeColecciones {
	...
	@Bean //Es el equivalente a @Component pero para métodos
	@Qualifier("getLista")
	List<Alumno> getAlumnoslist() {
		...
		alumnos.add(new Alumno("16790751", "LaurenTrantow", Arrays.asList(3, 3, 5, 1, 3, 5, 1, 5, 4, 6), 0f));
	...
	@Bean
	@Qualifier("getVacia")
	List<Alumno> getAlumnoEmpty() {
		List<Alumno> alumno = new ArrayList<>();
		alumno.add(new Alumno("18939934", "KenHaley", Arrays.asList(6, 7, 2, 1), 0f));
		return alumno ;
	}
}
